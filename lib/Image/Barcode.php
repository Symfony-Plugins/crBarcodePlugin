...
class Image_Barcode extends PEAR
{
     ...
     */
    static function getInstance($text, $type = 'int25', $imgtype = 'png', $bSendToBrowser = true)
    {
        //Make sure no bad files are included
        if (!preg_match('/^[a-zA-Z0-9_-]+$/', $type)) {
            return PEAR::raiseError('Invalid barcode type ' . $type);
        }
        if (!include_once(sfConfig::get('sf_plugins_dir').'/crBarcodePlugin/lib/Image/Barcode/' . $type . '.php')) {
        ...
        if ($img instanceof Pear_Error) {
            throw new Exception (!is_null($img)?$img->getMessage():'Error creating barcode');
        }
